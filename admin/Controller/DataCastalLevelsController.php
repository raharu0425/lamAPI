 ...
 */
class DataCastalLevelsController extends AdminBaseImpoterController {

    /**
     * 設定ファイル
     */
    //Dependecies
    public $model_names = array('DataCastalLevel');
    //CSVファイル名
    public $csv_file_name = "DataCastalLevel";
    //操作するマスター名
    public $display_model_name = 'DataCastalLevel';
    //パンくずリスト
    public $crumbs = array('マスター管理' => '/top/index', '拠点レベル管理' => '/DataCastalLevels/index');
    //スキップする最初の名前
    ...
    public function getCsv4ColumFields()
    {
        ...
        $db_column = array_keys($this->DataCastalLevel->getColumnTypes(false));
    ...
    public function creatDisplayRows($dto)
    {
        ...
        //現在の配列を取得
        $row = $this->DataCastalLevel->find('first', array('conditions' => array($this->display_model_name . '.' . $this->skip_waste . ' = ' . $dto[$this->skip_waste])));
    ...
    {
        $this->DataCastalLevel->save(array($this->display_model_name => $dto));
